...
int start_server(char *port) {
    ...
    int server_num = socket(info->ai_family, info->ai_socktype,
                            info->ai_protocol);
    if (server_num < 0) {
        perror("socket()");
        exit(1);
    ...
    // otherwise there is a wait time and bind will fail with
    // "Address already in use"
    int enable = 1;
    if (setsockopt(server_num, SOL_SOCKET, SO_REUSEADDR,
        &enable, sizeof(int)) < 0) {
...
void respond(int connection_desc) {
    ...
    if (read_status < 0) {
        ...
        if (strncmp(mesg, get_str, sizeof(get_str) - 1) == 0 &&
            ...
            send(connection_desc, image_server_data,
                 image_server_data_size, MSG_NOSIGNAL);
